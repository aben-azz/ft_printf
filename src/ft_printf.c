...
{
	display_fmt(fmt);
	// printf("________FLAGS CHAR_________\n");
	(void)list;
	printf("%c", va_arg(list, int(char)));
	(void)fmt;
	// printf("________FLAGS CHAR_________\n");
	/* code */
	return (0);
...
{
	display_fmt(fmt);
	//printf("________FLAGS CHAR*________\n");
	(void)list;
	//printf("String: |%s|\n", fmt.string);
...
{
	display_fmt(fmt);
	// printf("________FLAGS PTR__________\n");
	(void)list;
...
{
	display_fmt(fmt);
//	printf("________FLAGS FLOAT________\n");
	(void)list;
...
{
	display_fmt(fmt);
	//printf("________FLAGS INT__________\n");
	(void)list;
...
{
	display_fmt(fmt);
	// printf("________FLAGS UINT_________\n");
	(void)list;
...
{
	display_fmt(fmt);
	// printf("________FLAGS HIGHO_________\n");
	(void)list;
...
{
	display_fmt(fmt);
	// printf("________FLAGS LOWH_________\n");
	(void)list;
	printf("String: |%s|\n", fmt.string);
	(void)fmt;
	// printf("________FLAGS LOWH_________\n");
...
{
	display_fmt(fmt);
	// printf("________FLAGS HIGHX________\n");
	(void)list;
...
int		*count_flags(char *string, int index)
{
	...
	while (*(string + offset))
	{
		if (*(string + offset) == '%' && *(string + offset + 1) != '%')
			if(!values[1]++) (values[0] = offset++);
		offset += (*(string + offset) == '%') + 1;
	}
//	printf("cout_flag {%d,%d}\n", values[0], values[1]);
	return (values);
}
...
int		get_options(char *str)
{
	...
	s = 0;
	while (OPTIONS[++i])
		if(~ft_indexof(str, OPTIONS[i])) (s |= (1 << i));
	// printf("#:%d\n0:%d\n-:%d\n+:%d\n :%d, %d\n",
	// 	(s & HASH) > 0,
	// 	(s & ZERO) > 0,
	// 	(s & SUB) > 0,
...
t_fmt	*get_flags(char *s, int n)
{
	...
	//view_fmt(flags, count_flags(s, 0)[1]);
	return (flags);
}
...
// 	i = 0;
// 	(void)i;
// 	length = count_flags(format, 0)[1];
// 	printf("longueur de %d\n", length);
// 	get_flags(format, length);
// 	va_start(ap, format);
// 	// while (i < length)
// 	// {
// 	// 	printf("xd %i\n", i++);
